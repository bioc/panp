...
This document describes how to use \Rpackage{panp} to perform gene detection (make presence/absence calls) on AffyMetrix HG-U133 series microarray data. Currently, the HG-U133A and HG-U133 Plus 2.0 are supported. \Rpackage{panp} operates on preprocessed microarray expression data as an \Robject{exprSet} object. Any preprocessing method can be used to create the \Robject{exprSet}, such as \Rfunction{rma()}, \Rfunction{mas5()}, \Rfunction{expresso()}, or \Rfunction{gcrma()}. Examples show how to quickly create such an \Robject{exprSet}, then how to use \Rpackage{panp} to generate a set of presence/absence calls and a set of p-values indicating significance of the detection for each expression value.
...
First, the CEL file data must be preprocessed to generate expression values. This is done in two stages: create an AffyBatch object, then process that to generate expression values as an \Robject{exprSet}. In this example, we'll use \Rpackage{gcrma} for the latter, so let's load the required libraries for that and for  \Rpackage{panp} (note that loading these packages will automatically cause to be loaded any other packages they require, such as \Rpackage{affy}):
...
\subsection{Creating the \Robject{exprSet}}

The \Rfunction{gcrma} function is one way to do this.

\begin{verbatim}
> gcrma.exprSet <- gcrma(samples)
> # You might wish to save it:
> save(gcrma.exprSet, file= "gcrma.exprSet.Rdata")
\end{verbatim}
@
For this example, we have already done the above steps, using \Rpackage{gcrma} to create an \Robject{exprSet} with three samples, which we will now load:
...
Now, let's run it on our \Robject{exprSet}. We'll use the default p-value cutoffs of 0.01 and 0.02. So in this case, intensities above the intensity at the 0.01 cutoff will be called "P" (present); intensities between the two cutoffs will be assigned an "M" (marginal), and those below the intensity at the 0.02 p-value will get an "A"(absent).

<<run.pa.calls>>=
PAset <- pa.calls(gcrma.exprSet)
@
The screen output tells you what the intensity values are at each of the two cutoff p-values, for each of the three chips in our \Robject{exprSet}. (Be aware that some preprocessing methods, such as \Rfunction{rma} and \Rfunction{gcrma}, return the expression values in log(2) form. Others return untransformed expression values. \Rfunction{pa.calls()} works equally well in either case.) The final output line informs the user that if any warning messages about 'Collapsing to unique values' appear, they are benign (there are none in this example). The collapsing sometimes occurs when \Rfunction{pa.calls()} interpolates over a large data set, and is expected.

\section{Extracting P/M/A calls and p-values}

The presence/absence calls and p-values are embedded in the \Robject{exprs} and \Robject{se.exprs} slots of the returned \Robject{exprSet} (here, "PAset"), respectively. These two slots can now be extracted for further use in the R environment. They can also be saved as comma-separated files, in case it is desired to view them in Excel, for instance. (You can, of course, also save these as Rdata files for later use in the R environment.)

<<extract.PAcalls>>=
PAcalls <- exprs(PAset)
Pvalues <- se.exprs(PAset)
...
NSMP_x <- sort(exprs(gcrma.exprSet)[NSMPnames.hgu133a,1], decreasing=TRUE)
NSMP_y <- seq(0,1,1/(length(NSMP_x)-1))

# Plot expression densities of all probesets, then just NSMPs
plot(density(exprs(gcrma.exprSet)[,1]),
	col="blue",
	xlim = c(1,13),
	ylim = c(0,.8),
...
x = NSMP_x
curve(interp(x),add=TRUE, lwd=2)	# …and add it to the plot
...
lines(density(exprs(gcrma.exprSet)[,1][PAcalls[,1]=="P"], bw=.1, n=512),col=2, lty=2, lwd=1)
lines(density(exprs(gcrma.exprSet)[,1][PAcalls[,1]=="A"], bw=.1, n=512),col=3, lty=2, lwd=1)
...
of Gene Detection on Oligonucleotide Expression Arrays (2005), under review
